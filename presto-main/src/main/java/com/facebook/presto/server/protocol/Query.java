...
class Query
{
    ...
    public synchronized ListenableFuture<QueryResults> waitForResults(long token, UriInfo uriInfo, String scheme, Duration wait, DataSize targetResultSize, boolean useBinaryData)
    {
        ...
        return Futures.transform(futureStateChange, ignored -> getNextResultWithRetry(token, uriInfo, scheme, targetResultSize, useBinaryData), resultsProcessorExecutor);
    }
    ...
    private synchronized QueryResults getNextResultWithRetry(long token, UriInfo uriInfo, String scheme, DataSize targetResultSize, boolean useBinaryData)
    {
        QueryResults queryResults = getNextResult(token, uriInfo, scheme, targetResultSize, useBinaryData);
    ...
    private synchronized QueryResults getNextResult(long token, UriInfo uriInfo, String scheme, DataSize targetResultSize, boolean useBinaryData)
    {
        ...
        if (nextToken.isPresent()) {
            nextResultsUri = createNextResultsUri(scheme, uriInfo, nextToken.getAsLong(), useBinaryData);
        }
    ...
    {
        String newPath = binaryResults ? "/v1/binary/statement/executing" : "/v1/statement/executing";
