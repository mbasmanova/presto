        ...
        implements SelectiveStreamReader
{
            ...
            AggregatedMemoryContext systemMemoryContext)
    {
        ...
        switch (elementStreamDescriptor.getStreamType())
        {
            case LIST:
                this.elementStreamReader = new ListSelectiveStreamReader(elementStreamDescriptor, ImmutableMap.of(), ImmutableList.of(), this.listFilter != null ? (ListFilter) this.listFilter.getChild() : null, level + 1, elementOutputType, hiveStorageTimeZone, systemMemoryContext.newAggregatedMemoryContext());
                break;
            case MAP:
            case STRUCT:
            ...
            default:
                ...
                if (this.listFilter != null) {
                    PositionalFilter positionalFilter = this.listFilter.getPositionalFilter();
                    if (positionalFilter != null) {
                        elementFilters = ImmutableMap.of(new Subfield("c"), positionalFilter);
                    }
                }
                if (!outputRequired && elementFilters.isEmpty()) {
                    // No need to read the elements when output is not required and the filter is a simple IS [NOT] NULL
                    this.elementStreamReader = null;
                }
                else {
                    this.elementStreamReader = createStreamReader(elementStreamDescriptor, elementFilters, elementOutputType, ImmutableList.of(), hiveStorageTimeZone, systemMemoryContext.newAggregatedMemoryContext());
                }
        }
            ...
            throws IOException
    {
        ...
        if (outputRequired) {
            ensureOffsetsCapacity(positionCount);
        }

        if (nullsAllowed && presentStream != null && (outputRequired || listFilter != null)) {
            ensureNullsCapacity(positionCount);
        }
    ...
    private void populateOutputPositionsWithFilter(int elementPositionCount)
    {
        ...
        int elementOffset = 0;
        boolean[] positionsFailed = listFilter.getFailed();
        boolean[] indexOutOfBounds = listFilter.getIndexOutOfBounds();
        for (int i = 0; i < outputPositionCount; i++) {
    ...
    private void ensureOffsetsCapacity(int capacity)
    {
        offsets = ensureCapacity(offsets, capacity + 1);
    }

    private void ensureNullsCapacity(int capacity)
    {
    ...
    {
        outputPositions = ensureCapacity(outputPositions, capacity);
    }
    ...
    public BlockLease getBlockView(int[] positions, int positionCount)
    {
        ...
        return newLease(ArrayBlock.fromElementBlock(positionCount, Optional.ofNullable(includeNulls ? nulls : null), offsets, elementBlockLease.get()), () -> { elementBlockLease.close(); valuesInUse = false; });
    }
    ...
    {
        return INSTANCE_SIZE + sizeOf(offsets) + sizeOf(nulls) + sizeOf(outputPositions) +
