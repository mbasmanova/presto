        ...
        implements HierarchicalFilter
{
    ...
    public void populateElementFilters(int positionCount, boolean[] nulls, int[] lengths, int lengthSum)
    {
        ...
        if (parent == null) {   // top-level filter
            ...
            // update parentTopLevelOffsets to take into account failed offsets
            NullsFilter nullsFilter = parent.getNullsFilter();
            if (nullsFilter != null) {
                boolean[] chunkFailed = nullsFilter.getFailed();
                int skipped = 0;
            ...
            for (int i = 0; i < positionCount; i++) {
                ...
                for (int j = 0; j < lengths[i]; j++) {
                    if (subscriptFilters != null && j < subscriptFilters.length) {
                        elementFilters[elementPosition] = parentElementFilters[i] & subscriptFilters[j];
                    }
                    else {
                        elementFilters[elementPosition] = 0;
                    }
                ...
                if (subscriptFilters != null && (nulls == null || !nulls[i])) {
                    for (int j = lengths[i]; j < subscriptFilters.length; j++) {
                        if ((parentElementFilters[i] & subscriptFilters[j]) > 0) {
                            // this entry doesn't have enough elements for all filters,
                            // hence, raise "Array index out of bound" error
    ...
    private void setupPositionalFilter()
    {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (filter > 0) {
                elementTupleDomainFilters[i] = tupleDomainFilters[Long.numberOfTrailingZeros(filter)];
            }
            else {
    ...
    private void setupNullsFilter()
    {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (filter > 0) {
                TupleDomainFilter tupleDomainFilter = tupleDomainFilters[Long.numberOfTrailingZeros(filter)];
    ...
    public boolean[] getFailed()
    {
        ...
        boolean[] failed = child.getFailed();
        if (nullsFilter != null) {
    ...
    @Override
    public boolean[] getIndexOutOfBounds()
    {
        if (child == null) {
            return indexOutOfBounds;
        }

        boolean[] indexOutOfBounds = child.getIndexOutOfBounds();
    ...
    public long getRetainedSizeInBytes()
    {
        return INSTANCE_SIZE + sizeOf(elementFilters) + sizeOf(tupleDomainFilters) + sizeOf(subscriptFilters) +
                sizeOf(elementTupleDomainFilters) + sizeOf(topLevelOffsets) + sizeOf(nonNullsAllowed) +
                sizeOf(indexOutOfBounds) + sizeOf(nullsAllowed) +
    ...
    // TODO Move into separate class ArrayAllocations
